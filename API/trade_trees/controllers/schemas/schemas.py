...
class TradeTreeBranchAPISchema(Schema):
    discriminator = String(required=False, validate=OneOf(
        list(TradeTreeDiscriminator.__members__.keys())))
    discriminant = String()
    ...
    operation = String(
        validate=OneOf(
            choices=[
                *
                list(
                    TradeTreeSchemaOperation.__members__.keys()),
                *
                list(
                    TradeTreeTimeSeriesOperation.__members__.keys())]))
...
class TradeTreeBranchOutcome(Schema):
    operation = String(
        required=True, validate=OneOf(
            choices=list(
                TradeTreeOutcomeOperation.__members__.keys())))
...
class PostTradeTreeAPIInSchemaSuccess(Schema):
    title = String(
        ...
        validate=Length(
            ...
            50),
        metatdata={
            'description': 'The name of a trade tree.',
            'example': 'Flash'})
