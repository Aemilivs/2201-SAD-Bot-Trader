...
class TradeTreeRepository:
    def __init__(self, configuration, db) -> None:
        ...
        self.db.create_tables(
            [TradeTreeOutcome, TradeTreeRoot, TradeTreeBranch])
    ...
    def read_trade_tree(self, id):
        # TODO Make this query return a valid trade tree (with a
        # children/branch hierarchy).
        return TradeTreeRoot.select().join(
            TradeTreeBranch,
            on=TradeTreeBranch.root == TradeTreeRoot.id).join(
            TradeTreeOutcome,
            on=TradeTreeBranch.root == TradeTreeOutcome.root).where(
            TradeTreeRoot.id == id)

    def read_user_tree_roots(self, user_id):
        return TradeTreeRoot.select(TradeTreeRoot.id).where(TradeTreeRoot.user_id == user_id)
    ...
    def update_trade_tree(self, entity: TradeTreeRoot):
        ...
        ).where(TradeTreeRoot.id == entity.id and TradeTreeRoot.user_id == entity.user_id).execute()

    def delete_trade_tree(self, id, user_id):
        return TradeTreeRoot.delete().where(TradeTreeRoot.id == id and TradeTreeRoot.user_id == user_id).execute()
